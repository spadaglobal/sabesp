...
import FindLvPosService from '../FindLvPosService';
...
let updateLvPos: FindLvPosService;
...
describe('Find LvPos', () => {
  beforeEach(() => {
    ...
    updateLvPos = new FindLvPosService(fakeLvPosRepository);
  });
  it('should be able to return a lvPos', async () => {
    ...
    const lvPosFind = await updateLvPos.execute({
      lv_pos_id: lvPos.id,
    });

    expect(lvPosFind.id).toEqual(lvPos.id);
  });
  it('should not be able to return a non-exists lvPos', async () => {
    await expect(
      updateLvPos.execute({
